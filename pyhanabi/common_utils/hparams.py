...
from common_utils.multiprocess import main_process_only

global_print_hparams = True
hparams = {}


yaml.add_constructor('!eval', lambda loader, node: eval(node.value))
...
def load_config(config_fn):
        ...
        if 'base_config' in hparams_:
            ...
            for c in hparams_['base_config']:
                if c not in loaded_config:
                    ...
                    override_config(ret_hparams, _load(c))
    ...
    ret_hparams = _load(config_fn)
    return ret_hparams, config_chains
...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    ...
    if args_work_dir != '' and os.path.exists(ckpt_config_path):
        with open(ckpt_config_path, encoding='utf-8') as f:
            saved_hparams.update(yaml.load(f, yaml.Loader))

    if args.config != '':
        hparams_, config_chains = load_config(args.config)
    else:
        hparams_, config_chains = {}, []
    ...
    @rank_zero_only
    @main_process_only # Only the main process will save the config file
    def dump_hparams():
